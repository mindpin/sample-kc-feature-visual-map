...
(function() {
  ...
  Graph = (function() {
    ...
    Graph.prototype._draw_svg = function() {
      ...
      this.screen_w = 16000;
      this.screen_h = 8000;
      this.svg = d3.select('.graph-paper').append('svg').attr('width', this.screen_w).attr('height', this.screen_h);
      zoom_behavior = d3.behavior.zoom().scaleExtent([0.25, 2]).on("zoom", this._zoom);
      this.g1 = this.svg.append('g').call(zoom_behavior);
      this.g2 = this.svg.append('g');
      return this.g1.append("rect").attr("class", "overlay").attr('width', this.screen_w).attr('height', this.screen_h).style('fill', 'none').style('pointer-events', 'all');
    };

    Graph.prototype._zoom = function() {
      var scale, translate;
      ...
      return this.g2.attr('transform', "translate(" + (translate[0] + false) + ", " + translate[1] + ")scale(" + scale + ")");
    };
    ...
    Graph.prototype._draw_tree = function() {
      ...
      node_enter = this.g2.selectAll('.node').data(nodes).enter().append('g').attr('class', 'node').attr('transform', function(d) {
      ...
      return this.g2.selectAll('.link').data(links).enter().insert('path', 'g').attr('class', 'link').attr('d', diagonal).style('fill', 'none').style('stroke', '#6F7B7E').style('stroke-width', '3px').style('display', function(d) {
